...
export const DeckStats = ({ deck }: { deck: Deck }) => {
  const daysSinceCreation = deck.createdAt
    ? Math.floor(
        (new Date().getTime() - new Date(deck.createdAt).getTime()) /
          (1000 * 3600 * 24)
      )
    : 0;
  ...
  const daysSinceLastStudy = deck.progress?.lastStudied
    ? Math.floor(
        (new Date().getTime() - new Date(deck.progress.lastStudied).getTime()) /
          (1000 * 3600 * 24)
      )
    : null;
          ...
          <div className="flex-1">
            ...
            <div className="flex items-end gap-2">
              <span className="text-3xl font-bold">
                {deck.progress?.mastery.toFixed(1) || 0}%
              </span>
            ...
            {deck.progress?.lastStudied && (
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatDateTime(deck.progress.lastStudied)}
                  </p>
                </div>
              </div>
            )}
