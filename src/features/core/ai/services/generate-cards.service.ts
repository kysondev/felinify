...
Each flashcard should consist of a concise **term** (ideally a single vocabulary word or short phrase) and a clear, accurate **definition/answer**.
...
- The definiton/answer should be a word or 1–2 sentences max. No long explanations.
- No extra text. Return JSON only.
- Flashcard definiton/answer cannot contain word from flashcard question.`;
    ...
    const flashcards = parsed.map((card: { t: string; d: string }) => ({
      term: card.t,
      definiton: card.d,
    }));
