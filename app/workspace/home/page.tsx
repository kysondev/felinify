...
export default async function HomePage() {
  ...
  const totalCards = decks?.reduce((sum, deck) => sum + (deck.flashcards?.length || 0), 0) || 0;

  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  const recentlyStudiedCount = decks?.filter(deck => 
    deck.progress?.lastStudied && new Date(deck.progress.lastStudied) > sevenDaysAgo
  ).length || 0;
  ...
  ?.sort((a, b) => new Date(b.progress!.lastStudied!).getTime() - new Date(a.progress!.lastStudied!).getTime())
  ?.slice(0, 2) || [];
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center max-w-3xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
                <Card className="border border-border shadow-sm hover:shadow transition-all duration-300">
                  <CardContent className="p-6 flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Decks in Library</p>
                  </CardContent>
                </Card>
                ...
                <Card className="border border-border shadow-sm hover:shadow transition-all duration-300">
                  <CardContent className="p-6 flex flex-col items-center text-center">
                    ...
                    <h3 className="text-2xl font-bold mb-1">{recentlyStudiedCount}</h3>
                    <p className="text-sm text-muted-foreground">Recently Studied</p>
                  </CardContent>
                </Card>
              </div>
            </>
            ...
            <Card className="w-full mb-12 border border-dashed border-primary/30 bg-primary/5">
              <CardContent className="p-8 flex flex-col items-center text-center">
                ...
                <p className="text-muted-foreground mb-6 max-w-md">
                  Start your learning journey by creating your first flashcard deck or explore our collection of ready-made decks.
                </p>
          ...
          {recentDecks.length > 0 && (
            <div className="w-full mb-8">
              <h2 className="text-lg font-semibold mb-4 text-left">Continue Learning</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {recentDecks.map((deck, index) => (
                  <Link href={`/workspace/deck/edit/${deck.id}`} key={deck.id}>
                  <Card key={deck.id} className="group border cursor-pointer border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 transition-all duration-300 hover:border-primary/30">
                    <CardContent className="p-4 flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <span>
                          {new Date(deck.progress!.lastStudied!).toLocaleDateString()}
                        </span>
                        </div>
                      </CardContent>
                    </Card>
